# Vansh Grover

**Director of Product Design | AI Engineer | Frontend Developer**

> *Bridging the physical and digital worlds through design.*

---

## About Me

Hi! I'm Vansh Grover, a multidisciplinary designer and engineer with a unique background in **Physics** and **User Experience Design**. I operate at the intersection of analytical rigor and creative empathy, building intelligent products that are both beautiful and functional.

Currently, I am focused on **Spatial Computing** and **Ambient Intelligence**, exploring how technology can anticipate our needs and enhance our capabilities without getting in the way.

## Tech Stack & Skills

### 🎨 Design
*   **Tools**: Figma, Framer, Spline, Rive, Adobe Creative Suite
*   **Methodologies**: Atomic Design Systems, User-Centered Design, Prototyping, 3D Modeling

### 💻 Frontend Engineering
*   **Core**: React, TypeScript, Next.js, HTML5, CSS3
*   **Styling**: Tailwind CSS, Framer Motion, GSAP

### 🤖 AI & Machine Learning
*   **Languages**: Python
*   **Technologies**: RAG Systems, LangChain, Vector Databases (pgvector), LLM Integration
*   **Focus**: Generative Engine Optimization (GEO), Agentic Workflows

### ⚙️ Backend & Infrastructure
*   **Stack**: Node.js, tRPC, Supabase, PostgreSQL

## Experience Highlights

*   **Director of Product Design @ DriQ Health** *(Feb 2025 - Oct 2025)*
    *   Led product strategy and digital transformation, increasing daily unique visitors by 70x.
    *   Spearheaded Generative Engine Optimization (GEO) strategies.

*   **Product & Brand Designer @ Sparto** *(Apr 2025 - Jul 2025)*
    *   Designed the end-to-end mobile app and enterprise admin panel for an automotive spare parts platform.

*   **Assistant Professor @ DIT University** *(Jul 2023 - Sep 2024)*
    *   Mentored students across Physics, Robotics, and Web Development.

## Certifications

*   **Post Graduate Program in AI and Machine Learning** - Caltech University (2025)
*   **Python for Data Science** - IBM
*   **CS50's Introduction to Computer Science** - HarvardX
*   **Brand Management: Aligning Business, Brand and Behavior** - London Business School
*   **Figma to Lottie Certification** - Lottie Files

## Education

*   **Post Graduate Program in AI and Machine Learning** | Caltech University (2025)
*   **Master of Design (M.Des) in User Experience Design** | IT University (2020 - 2022)
*   **B.Sc. Honors in Physics** | Sri Venkateswara College, University of Delhi (2017 - 2020)

---

[Portfolio Website](https://portfolio.vansh.fyi)
