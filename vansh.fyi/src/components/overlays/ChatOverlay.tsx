...
interface Message {
  sender: 'user' | 'ai';
  text: string;
  isNew?: boolean; // Flag to trigger streaming animation for new AI messages
}
...
const ChatView: React.FC = () => {
  ...
  // Smooth scroll to bottom - used during streaming
  ...
  useEffect(() => {
    ...
    if (data && data.response) {
      setMessages((prev) => {
        // Mark previous messages as not new (so they don't re-stream)
        const updated = prev.map(m => ({ ...m, isNew: false }));
        return [...updated, { sender: 'ai', text: data.response, isNew: true }];
      });
      setPendingQuery(''); // Clear pending query after receiving response
    } else if (error) {
      setMessages((prev) => {
        const updated = prev.map(m => ({ ...m, isNew: false }));
        return [...updated, { sender: 'ai', text: 'Ursa is having trouble connecting. Please try again.', isNew: true }];
      });
                    ...
                    {isSending && (
                      <div className="flex gap-3 gap-x-3 gap-y-3 items-center">
                        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-white/10 ring-1 ring-white/10 text-white">
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="w-[16px] h-[32px] flex-shrink-0" strokeWidth="2" data-icon-replaced="true">
                            <path opacity="0.5" fillRule="evenodd" clipRule="evenodd" d="M11.8401 2.3723C12.1332 2.32759 12.4306 2.29559 12.7316 2.27688C12.5229 2.26391 12.3125 2.25732 12.1005 2.25732C11.8001 2.25732 11.5027 2.27055 11.209 2.29645C11.4213 2.31518 11.6317 2.34053 11.8401 2.3723Z" />
                            <path fillRule="evenodd" clipRule="evenodd" d="M7.13151 9.67024C7.25322 9.67024 7.35188 9.57157 7.35188 9.44987C7.35188 9.32816 7.25322 9.22949 7.13151 9.22949C7.0098 9.22949 6.91113 9.32816 6.91113 9.44987C6.91113 9.57157 7.0098 9.67024 7.13151 9.67024Z" />
                            <path fillRule="evenodd" clipRule="evenodd" d="M16.0075 9.67024C16.1292 9.67024 16.2279 9.57157 16.2279 9.44987C16.2279 9.32816 16.1292 9.22949 16.0075 9.22949C15.8858 9.22949 15.7871 9.32816 15.7871 9.44987C15.7871 9.57157 15.8858 9.67024 16.0075 9.67024Z" />
                            <path fillRule="evenodd" clipRule="evenodd" d="M17.8295 21.2188C21.1165 19.6059 23.3799 16.2044 23.3799 12.2743C23.3799 10.452 22.8933 8.74331 22.0429 7.27116C21.8683 6.96891 21.6784 6.67664 21.4742 6.3954C21.0893 5.86527 20.6537 5.37433 20.1745 4.92972C19.9193 4.69291 19.6517 4.46923 19.3728 4.25978C17.6987 3.00239 15.6178 2.25732 13.3629 2.25732C13.151 2.25732 12.9405 2.26391 12.7318 2.27688C12.4309 2.29559 12.1335 2.32759 11.8403 2.3723C11.632 2.34053 11.4216 2.31518 11.2093 2.29645C10.9156 2.27055 10.6182 2.25732 10.3178 2.25732C8.17639 2.25732 6.19196 2.92925 4.5638 4.07377C4.29986 4.2593 4.04529 4.45726 3.80092 4.66678C3.20434 5.17831 2.66862 5.7588 2.20612 6.3959C2.01715 6.65621 1.84041 6.92597 1.67673 7.20434C0.802273 8.69151 0.300781 10.4243 0.300781 12.2743C0.300781 16.2044 2.56414 19.6059 5.8585 21.2464" />
                          </svg>
                        </div>
                        <ThinkingLoader />
                      </div>
